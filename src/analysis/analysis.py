...
class Result:
    ...
    def plot_hist(self,arrays,filename,title)->None:
        ...
        fig.savefig(filename,transparent=False,format="svg")

    def write_results(self,arrays,filename,title):
        if self.load_first_time:
            with open(filename,"w",newline="") as file:    
                writer =csv.writer(file)
                writer.writerow(["Title", "Params", "Mean", "Median", "std", "variance", "min", "max"])
...
MASTER.write_results(MASTER.arrays_bg1,"results.csv","Background 1")
MASTER.write_results(MASTER.arrays_bg2,"results.csv","Background 2")
MASTER.write_results(MASTER.arrays_bg3,"results.csv","Background 3")
MASTER.write_results(MASTER.arrays_bg4,"results.csv","Background 4")
MASTER.write_results(MASTER.arrays_s1,"results.csv","Sample 1")
MASTER.write_results(MASTER.arrays_s2,"results.csv","Sample 2")